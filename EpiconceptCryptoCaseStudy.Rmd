...
You will need to install these before starting. We will be using the following package. 
...
# Installing multiple required packages for the case study
  #see appendix for using xlsx to read excel sheets (dont run this now)
required_packages <- c("epiR", "xlsx")
...
cryptoreg <- reshape(cryptoreg, idvar = "year", timevar = "region", direction = "wide")

```

Then you  can create graphs (it also easier to export to Excel): 
```{r}
